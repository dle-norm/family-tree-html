...
function html(filename, data) {
  ...
  for (const name in data.people) {
    let classname = data.people[name].classname
      ? data.people[name].classname
      : "";
    let fullname = data.people[name].fullname ? data.people[name].fullname : "";
    let dateBorn = data.people[name].born ? "*" + data.people[name].born : "";
    let dateDied = data.people[name].died
      ? "&#10013;" + data.people[name].died
      : "";
    ...
    if (house) {
      ...
      $member.append(
        '<a class="parents-item-link" href="' +
          house +
          '.html">' +
          house +
          "</a>"
  ...
  data.families.forEach((el) => {
    ...
    if (!fatherHasChildren) {
      ...
      if (el.children && el.children[0] && el.parents[0]) {
      ...
      if (el.children && el.children[0] && el.parents[1]) {
  ...
  $(".parents").each(function (index) {
    if ($(this).find(".parents-item").length == 0) {
      ...
      let $wife = $("#" + wifeId);
      let $content = $('[data-id="' + wifeId + '"]')
        .closest(".flow-dendrogram")
        .children();
      $wife.after($content);
      $wife.remove();
    }
